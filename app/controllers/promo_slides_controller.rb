class PromoSlidesController < ApplicationController
  ...
  def destroy
    ...
    promo_slide.destroy
    render json: { slider_html: render_to_string('home/_promo_slider'), notice: 'Слайд удален' }
  end

  def update
    promo_slide = PromoSlide.find params[:id]
    promo_slide.update_attributes build_params
    render json: { slider_html: render_to_string('home/_promo_slider'), notice: 'Слайд изменен', goto: PromoSlide.all.index(promo_slide) }
  end

  def create
    promo_slide = PromoSlide.create(build_params)
    promo_slide.save
    render json: { slider_html: render_to_string('home/_promo_slider'), notice: 'Слайд добавлен', goto: PromoSlide.all.index(promo_slide) }
  end
